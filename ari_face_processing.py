...
# === Optional: Animate Talking Effect Preview ===
import tkinter as tk
from PIL import ImageTk

frames = ["neutral", "neutral_speaking", "neutral", "neutral_speaking"]

def animate_talking():
    def loop(i=0):
        img = Image.open(os.path.join(output_dir, f"ari_{frames[i]}.png"))
        tk_img = ImageTk.PhotoImage(img)
        label.config(image=tk_img)
        label.image = tk_img
        root.after(400, loop, (i + 1) % len(frames))
    loop()
